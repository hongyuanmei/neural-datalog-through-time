...
class Eval(object):
    ...
    def run(self):
        ...
        print(f"start evaluting on {self.args.Split} data of domain {self.args.Domain} ... ")
        bs = Bootstrapping()

        print(f"reading saved predictions and ground-truth ... ")
        with open(self.args.PathResult, 'rb') as f:
            all_res = pickle.load(f)

        # eval with loglik 
        print("eval on loglik")
        ...
        if 'loglik_token' in all_res: 
            ...
            print(f"avg. token-level loglik is {self.macro_avg(all_res['loglik_token']):.4f}")
            print(f"bootstrapping for significant tests")
